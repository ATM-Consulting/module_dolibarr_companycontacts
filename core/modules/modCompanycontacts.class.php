...
class modCompanycontacts extends DolibarrModules
{
    ...
    public function __construct($db)
    {
        ...
        $this->tabs = array(
        	...
        	'contact:+companycontacts:ThirdpartyLinked:companycontacts@companycontacts:$user->rights->companycontacts->read:/companycontacts/contacts_company.php?id=__ID__',
        ...
        if (! isset($conf->companycontacts->enabled)) {
        	$conf->companycontacts = ( object ) array ();
        ...
        $this->dictionnaries=array(
        		...
        		'tabname'=>array(MAIN_DB_PREFIX."c_contact_function",MAIN_DB_PREFIX."c_contact_department"),		// List of tables we want to see into dictonnary editor
        		'tablib'=>array("ContactFunction","ContactDepartment"),													// Label of tables
        		'tabsql'=>array('SELECT f.rowid as rowid, f.pos, f.code, f.label, f.active FROM '.MAIN_DB_PREFIX.'c_contact_function as f','SELECT f.rowid as rowid, f.pos, f.code, f.label, f.active FROM '.MAIN_DB_PREFIX.'c_contact_department as f'),	// Request to select fields
        		'tabsqlsort'=>array("pos ASC","pos ASC"),																					// Sort order
        		'tabfield'=>array("pos,code,label","pos,code,label"),																					// List of fields (result of select to show dictionnary)
        		'tabfieldvalue'=>array("pos,code,label","pos,code,label"),																				// List of fields (list of fields to edit a record)
        		'tabfieldinsert'=>array("pos,code,label","pos,code,label"),																			// List of fields (list of fields for insert)
        		'tabrowid'=>array("rowid","rowid"),																									// Name of columns with primary key (try to always name it 'rowid')
        		'tabcond'=>array($conf->companycontacts->enabled,$conf->companycontacts->enabled)												// Condition to show each dictionnary
        );
        /* Example:
